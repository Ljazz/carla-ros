...
try:
    data = pathlib.Path(
        "/home/realai/zhujianwei/vehicle_automation/carla/Unreal/CarlaUE4/Content/Carla/Maps/OpenDrive/Town01.xodr").read_text()
except OSError:
    sys.exit()
...
try:
    ...
    # 生成 50 辆随机分布在地图上的车辆
    for _ in range(50):
        v_actor = world.try_spawn_actor(random.choice(
            vehicle_blueprints, random.choices(spawn_points)))
    ...
    camera = world.spawn_actor(
        camera_bp, camera_init_trans, attach_to=ego_vehicle)
    actor_list.append(camera)
    camera.listen(lambda image: image.save_to_disk(
        'out/%06d.png' % image.frame))
